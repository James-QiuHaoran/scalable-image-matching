...
class SignatureES(SignatureDatabaseBase):
    ...
    def search_single_record(self, rec, pre_filter=None):
        ...
        # tmp = path[:path.rfind('_')]
        # batch = tmp[tmp.rfind('_')+1:]
        # direction = path[path.find('.')-1:path.find('.')]
    ...
    def insert_single_record(self, rec, refresh_after=False, batch=-1, direction=-1):
        rec['timestamp'] = datetime.now()
        rec['batch'] = batch
        rec['direction'] = direction
        self.es.index(index=self.index, doc_type=self.doc_type, body=rec, refresh=refresh_after)
        # ret = self.es.index(index=self.index, doc_type=self.doc_type, body=rec, refresh=refresh_after)
        # ret['_id'] used for delete
        # delete: self.es.delete(index=self.index, doc_type=self.doc_type, id=ret['_id'])
