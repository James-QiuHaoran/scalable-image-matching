...
class SignatureDatabaseBase(object):
    ...
    def add_image(self, path, img=None, bytestream=False, metadata=None, refresh_after=False):
        ...
        self.insert_single_record(rec, refresh_after=refresh_after, batch=rec['batch'], direction=rec['direction'])
...
def make_record(path, gis, k, N, img=None, bytestream=False, metadata=None):
    ...
    Returns:
        An image record.python get argument from command line
...
n get argument from command line
         'metadata': {...}
         }
    ...
    if isinstance(path, str):
        ...
        record['batch'] = tmp[tmp.rfind('_')+1:]
        record['direction'] = path[path.find('.')-1:path.find('.')] 
    if img is not None:
